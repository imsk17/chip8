...
/// Each character is made up of eight rows of five pixels
/// with each row using a byte of data
...
impl Emu {
    ...
    fn execute(&mut self, op: u16) {
        ...
        match (digit1, digit2, digit3, digit4) {
            ...
            (0, 0, 0xe, 0xe) => {
                let ret_addr = self.pop_from_stack();
                self.pc = ret_addr;
            }
            // 1NNN - Jump
            (1, _, _, _) => {
            ...
            // 3XNN - Skip next if VX == NN, SKIP VX == NN
            (3, _, _, _) => {
    ...
    pub fn load(&mut self, data: &[u8]) {
        ...
        let end = (Self::START_ADDR as usize) + data.len();
        self.ram[start..end].copy_from_slice(data);
    }

    /// The fetch function will only be called internally as part of our
    /// tick loop, so it doesn’t need to be public. The purpose of this
    /// function is to grab the instruction we are about to execute (known as an opcode)
